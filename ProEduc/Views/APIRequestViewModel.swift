...
class UserAPIRequest: ObservableObject {
    ...
    func fetchedUser() async -> [Eleve] {
        var resultEleve = [Eleve]()
        ...
        guard let url = URL(string: "https://api.airtable.com/v0/appx5SW9jo6zJQImo/Eleve") else {
        ...
        do {
            ...
            let decodedRequest = try JSONDecoder().decode(EleveRequete.self, from: data)
            ...
            decodedRequest.records.forEach { record in
                resultEleve.append(record.fields)
            }
